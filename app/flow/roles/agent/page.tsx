...
export default function AgentRolePage() {
  ...
  const fetchAgents = async () => {
    if (!tenantId) {
      console.error('❌ Cannot fetch agents: No tenant ID available')
      toast.error('No tenant ID found. Please sign in again.')
      return
    }
    
    console.log('✅ Fetching agents with tenant ID:', tenantId)
    setUserLoading(true)
    try {
      ...
      if (response.ok) {
        ...
        toast.error(`Failed to fetch agents: ${response.status}`)
      }
    } catch (error) {
  ...
  const handleCreateSubmit = async (e: React.FormEvent) => {
    ...
    console.log('🔍 Create agent form submission check:')
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error(errorData.detail || 'Failed to create agent')
      }
    } catch (error) {
  ...
  // Handle update agent form submission
  const handleUpdateSubmit = async (e: React.FormEvent) => {
    ...
    console.log('🔍 Update agent form submission check:')
    console.log('  - Agent ID:', editingUser.id)
    console.log('  - Form Data:', formData)
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response.json()
        toast.error(errorData.detail || 'Failed to update agent')
      }
    } catch (error) {
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-2">
            Create, edit, and manage agents within your organization
          </p>
        </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {agents.filter(u => u.is_active).length}
            </div>
            <p className="text-xs text-muted-foreground">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Create, edit, and manage agents in your organization
              </p>
            </div>
            {/* Create Agent Modal */}

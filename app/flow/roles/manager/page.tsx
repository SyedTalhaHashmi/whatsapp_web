...
export default function ManagerRolePage() {
  ...
  const fetchManagers = async () => {
    ...
    console.log('✅ Fetching managers with tenant ID:', tenantId)
    setUserLoading(true)
    try {
      ...
      if (response.ok) {
        const data = await response.json()
        // Filter only managers (role_id = 4)
        const managersData = data.filter((user: User) => user.role_id === 4)
        setManagers(managersData)
        console.log('✅ Managers fetched successfully:', managersData.length, 'managers')
      } else {
        console.error('❌ Failed to fetch users, status:', response.status)
        toast.error(`Failed to fetch managers: ${response.status}`)
      }
    } catch (error) {
    ...
    try {
      ...
      const response = await fetch(`${getBaseUrl()}/user/signup`, {
        ...
        body: JSON.stringify(payload)
         ...
         toast.error(errorData.detail || 'Failed to create manager')
       }
         } catch (error) {
  ...
  const handleUpdateSubmit = async (e: React.FormEvent) => {
    ...
    if (!editingUser) {
      toast.error('No manager selected for editing')
      return
    }
    
    console.log('🔍 Update manager form submission check:')
    console.log('  - Manager ID:', editingUser.id)
    console.log('  - Form Data:', formData)

    try {
      const payload = {
        name: formData.name,
        role_id: 4, // Manager role ID (cannot be changed)
        department_id: parseInt(formData.department_id),
      ...
      console.log('Updating manager with payload:', payload)
      ...
      const response = await fetch(`${getBaseUrl()}/user/${editingUser.id}`, {
        ...
        body: JSON.stringify(payload)
         ...
         const errorData = await response.json()
         toast.error(errorData.detail || 'Failed to update manager')
       }
    } catch (error) {
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
           ...
           <p className="text-gray-600 dark:text-gray-400 mt-2">
             Create, edit, and manage managers within your organization
           </p>
         </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
           ...
           <CardContent>
             ...
             <p className="text-xs text-muted-foreground">
               Active managers in organization
             </p>
           </CardContent>
         </Card>
           ...
           <CardContent>
             <div className="text-2xl font-bold">
               {managers.filter(u => u.is_active).length}
             </div>
             <p className="text-xs text-muted-foreground">
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                         <div>
               <CardTitle className="text-xl">Manager Management</CardTitle>
               <p className="text-sm text-muted-foreground">
                 Create, edit, and manage managers in your organization
               </p>
             </div>
            {/* Create User Modal */}

...
export default function TeamRolePage() {
  ...
  const fetchTeam = async () => {
    ...
    console.log('✅ Fetching team with tenant ID:', tenantId)
    setUserLoading(true)
    try {
      ...
      if (response.ok) {
        ...
        toast.error(`Failed to fetch team: ${response.status}`)
      }
    } catch (error) {
      ...
      const response = await fetch(`${getBaseUrl()}/user/signup`, {
        ...
        body: JSON.stringify(payload)
         ...
         const errorData = await response.json()
            toast.error(errorData.detail || 'Failed to create team')      
        }
         } catch (error) {
  ...
  const handleUpdateSubmit = async (e: React.FormEvent) => {
    ...
    console.log('🔍 Update team form submission check:')
    console.log('  - Team ID:', editingUser.id)
    console.log('  - Form Data:', formData)
    ...
    try {
      ...
      const response = await fetch(`${getBaseUrl()}/user/${editingUser.id}`, {
        ...
        fetchTeam()
         ...
         const errorData = await response.json()
         toast.error(errorData.detail || 'Failed to update team')
       }
    } catch (error) {
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
           ...
           <p className="text-gray-600 dark:text-gray-400 mt-2">
             Create, edit, and manage team within your organization
           </p>
         </div>
        ...
        <CardHeader>
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
               ...
               <p className="text-sm text-muted-foreground">
                 Create, edit, and manage team in your organization
               </p>
             </div>
            {/* Create User Modal */}
